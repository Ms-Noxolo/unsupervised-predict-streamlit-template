...
avg_ratings_markdown = """
    The above distribution is of average movie ratings in the database, to avoid the case whereby
    a movie has one viewer with a rating of 5 for example, the data was filtered such that the movies
    considered were those with 10 or more viewers.  
"""
...
introduction_overview = """
    ...
    Simple recommenders: These are offer generalized recommendations to every user, based on movie popularity and/or genre.
    Content-based recommenders: These suggest similar items based on a particular item. This system uses item metadata, such as genre, director, description, actors, etc. for movies, to make these recommendations.
...
systems = """
    ...
    To achieve a Content-based engine, we considered pairwise cosine 
    ...
    -Get the list of cosine similarity scores for that particular movie with all movies,  Convert it into a list of tuples with the first element is its position, and the second is the similarity score.
    ...
    -
    The quality of any recommender would be increased with the usage of better metadata and by capturing more of the finer details.
...
Collaborative = """
    ...
    User-based Filtering: These are systems that recommend products to a user that similar users have liked.
    Item-based Filtering: These systems are extremely similar to the content recommendation engine that you built. These systems identify similar items based on how people have rated it in the past.
"""